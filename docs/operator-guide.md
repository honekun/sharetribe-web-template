...
1. **Open the tool** — go to `[your marketplace URL]/admin/bulk-import`, or click the blue **"Bulk
   ...
   password to enter.
2. **Open the template** — click "Abrir plantilla" to open the seller template in Google Drive, in a
   new tab. Use Drive's own download button there to save your copy. It is pre-filled with the
   current machine-readable headers (`title`, `description`, `price`, `pub_*`, and `imagen_*`) plus
   one example row. The "Ver ejemplo de ZIP" link in the help bar downloads a complete,
   ready-to-upload example ZIP (a CSV plus matching images) you can open to see exactly how a
   finished import looks. Spanish Google Sheets headers remain accepted only as compatibility
...
> **Template headers.** The Drive-hosted template uses the field
...
| `BulkImportPage.downloadTemplate` | `Open template`                                                | `Abrir plantilla`                                                      | Opens the Drive-hosted template in a new tab.                         |
